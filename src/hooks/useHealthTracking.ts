        ...
        useCallback(() => {
            ...
            const startCyclingTracking = async () => {
                try {
                    ...
                    if (!isActive) return;
                    const initialPoint = { latitude: initialPosition.coords.latitude, longitude: initialPosition.coords.longitude };
                    ...
                    cyclingWatcherRef.current = await Location.watchPositionAsync({
                        ...
                        if (!isActive) return;
                        const newPoint = { latitude: update.coords.latitude, longitude: update.coords.longitude };
                        setCyclingRoute((prev) => {
                            const next = [...prev, newPoint];
                            return next.length > 60 ? next.slice(next.length - 60) : next;
                        });
                        setCyclingRegion((prev) => prev ? { ...prev, ...newPoint } : { ...newPoint, latitudeDelta: 0.005, longitudeDelta: 0.005 });
                    });
                } catch {
